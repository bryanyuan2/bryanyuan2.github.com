...
const AwardsList: React.FC<AwardsListProps> = ({ awards = [] }) => {
    ...
    awardsArray.push(icon + '<strong>Internal Awards</strong>');

    if (awards) {
        awards.forEach((content: string) => {
            awardsArray.push('<li>' + content + '</li>');
        });
    }
    
    const awardsContent = awardsArray.join(' ');
